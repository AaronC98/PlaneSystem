...
# endif
	...
	void
	merge(multimap<_Key, _Tp, _Compare2, _Alloc>& __source)
	{
	  using _Merge_helper = _Rb_tree_merge_helper<multimap, _Compare2>;
	...
	{ merge(__source); }
      ...
      template<typename _Compare2>
	void
	merge(map<_Key, _Tp, _Compare2, _Alloc>& __source)
	{
	  using _Merge_helper = _Rb_tree_merge_helper<multimap, _Compare2>;
	  _M_t._M_merge_equal(_Merge_helper::_S_get_tree(__source));
	}
