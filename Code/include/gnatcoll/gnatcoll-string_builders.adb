...
package body GNATCOLL.String_Builders is
   ...
   function As_CString (Self : String_Builder) return CString is
   begin
      if Self.Str_Last = 0 then
         return Empty_CString;
      elsif Self.Str_Last > String_Builder_Short_Size then
         return CString (Self.Heap_Str (1)'Address);
      else
         return CString (Self.Stack_Str (1)'Address);
      end if;
   end As_CString;

   function As_CString (Self : Static_String_Builder) return CString is
   begin
      if Self.Str_Last = 0 then
         return Empty_CString;
      else
         return CString (Self.Str (1)'Address);
      end if;
   end As_CString;
